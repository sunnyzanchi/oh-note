...
import loadFromStrings from '../utils/loadFromStrings'
...
const filterNotes = (selectedTag: string, noteMap: NoteMap): NoteMap => {
  if (!selectedTag || selectedTag === 'All Tags') {
    return noteMap
  }

  return noteMap.filter((note, noteId) => {
...
const App = () => {
  ...
  const filteredNotes = filterNotes(selectedTag, noteMap)
